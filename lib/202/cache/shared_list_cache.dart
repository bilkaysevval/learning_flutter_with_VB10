...
class _SharedListCacheState extends LoadingStateful<SharedListCache> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: isLoading ? CircularProgressIndicator(color: Theme.of(context).highlightColor) : null,
      ...
      body: _UserListView(users: _users), // defined as follows
    );
  }
}
      ...
      itemBuilder: (BuildContext context, int index) {
        return Card(
          child: ListTile(
            ...
            subtitle: Text(users[index].description ?? 'description'),
            trailing: Text(users[index].url ?? 'url',
